...
- Decision 2026-07-10 production-release-certification: Certify production directly before numbering releases
    Decision: Superseded by `production-release-transaction` on 2026-08-28.
...
- Decision 2026-08-28 production-release-transaction: Prepare before deploy and roll back until publication
    Decision: Without a staging environment, production releases use a protected saga: certify the exact `main` SHA; prepare an unpublished exact-candidate GitHub draft with byte-verified assets; capture the healthy Worker; deploy and smoke-test; then publish the draft as the commit point. Any failure after deployment begins but before publication is proven restores and verifies the captured Worker. Homebrew automation is idempotent post-publication distribution. An exceptional protected repair may publish an already-deployed candidate only by validating the failed source run, reusing its unexpired certified artifact without rebuilding or redeploying, and proving that exact SHA remains live before and immediately before publication.
    Reason: GitHub drafts may not create their tag until publication, and deployment plus GitHub publication cannot be one cross-provider atomic transaction. Preparing every reversible artifact first and compensating every pre-commit failure prevents an unnumbered candidate from remaining in production.
    Tradeoffs: Drafts and backward-compatible migrations can exist before publication, and post-publication distribution can require retry, but users never receive a public release before all certified assets exist and production never remains on an uncommitted candidate after a known pre-publication failure.
