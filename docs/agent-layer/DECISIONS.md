...
- Decision 2026-08-27 predeploy-project-owned-homebrew-cask: Certify project-rendered Homebrew artifacts before production deployment
    Decision: Keep pinned GoReleaser as the archive/checksum builder, render the cask deterministically through `cli/internal/tools/rendercask`, and require exact release packaging plus Ruby/Homebrew style validation before the protected production deploy. Pass the same workflow artifact to post-deploy GitHub Release upload and guarded tap publication.
    Reason: The first public CLI release exposed two post-deploy failure classes: transient Go module downloads while rebuilding CLI artifacts and GoReleaser cask output incompatible with the tap's current Homebrew style rules. Agent Layer and Personal Context already demonstrate project-owned Homebrew rendering. Post-deploy upload and tap failures are retried through **Re-run failed jobs** on the original workflow run so they reuse its certified artifact; a fresh dispatch is not byte-identical because the binary embeds its build date.
    Tradeoffs: Release certification gains one artifact-transfer boundary and a Homebrew setup step, but packaging and cask compatibility can no longer create a partial release after production changes. GitHub upload, anonymous download, and tap-service failures remain recoverable from the original run only during the certified artifact's seven-day retention window; recovery after expiry requires a new numbered release.
...
- Decision 2026-08-29 publicly-atomic-release: Fail pre-commit releases without user-visible residue
    Decision: Production releases are one publicly atomic transaction identified by GitHub release ID. Certify the exact `main` SHA; create or adopt one owned draft; byte-verify CLI assets against that ID; upload an inactive Worker version; apply forward-only migrations; smoke the candidate at 0% through `Cloudflare-Workers-Version-Overrides` on the production hostname; promote it; mark the draft `publishing`; then publish by release ID. Publication, including reconcile retry of a `publishing` draft, re-proves certified CLI inventory and bytes immediately before `draft: false`. Artifact-less reconciliation holds rather than publishing GitHub draft bytes; recover with Re-run failed jobs on the original deploy so it reuses the certified artifact. Proven publication plus the numbered tag is the commit point. A proven pre-commit failure restores the previous Worker to 100% and deletes only that run's owned `prepared` draft. A `publishing` or published release is never automatically deleted or rolled back. Manual reconciliation under `production-deploy` is the mutating recovery path; scheduled detection is read-only.
    Reason: GitHub permits multiple drafts with the same tag name, so tag lookup is not a unique identity. `wrangler deploy` cannot stage a 0% candidate, while versions upload plus version overrides can smoke through the production hostname without preview URLs. The 2026-08-28 repair-oriented saga left unowned drafts, used tag-oriented lookup, and retained ambiguous outcomes for a separate repair publisher; that failed in run `33196586800` and left orphan draft `378670392` beside the later published `v0.2.7`.
    Tradeoffs: Inactive Worker versions, compatible expand migrations, Sentry records, Actions artifacts, and provider audit residue may remain after a pre-commit failure, but users never see candidate traffic, a tag, a published release, public assets, a Homebrew change, or an owned draft, and the numbered version stays reusable. Hold state after a lost publish requires an operator, and route/cron changes are blocked from the application release.
...
- Decision 2026-08-22 release-marketing-screenshot-attestation: Recapture and approve landing screenshots before every version bump
    Decision: `marketing/screenshots.json` is the canonical inventory for landing-page product screenshots and records the release version, capture route and viewport, and approved SHA-256. Release preparation first fetches numbered tags and rejects a target older than the package or fetched `main` version, not newer than the latest tag, or whose tag exists. Equality with the package and `main` version is allowed so an unpublished prepared release can be resumed. It then regenerates candidate images from the deterministic human-review fixture using a pinned Linux/amd64 Playwright container. A pixel tolerance gate discards insignificant raster drift and copies only substantive candidates into `public/` for review. Only comparison evidence is temporary. Release verification recaptures and compares without modifying committed files, while production preparation fails fast on a stale version or substantive pixel difference.
    Reason: Real product screenshots are release material and substantive changes must be intentionally reviewed, but rasterizer drift must not manufacture meaningless binary changes. The gate allows a maximum channel delta of 5 across at most 0.1% of pixels, while the pinned rasterizer keeps local Apple Silicon and Linux CI evidence comparable.
    Tradeoffs: Screenshot verification adds a Docker-backed Playwright capture to release certification and an explicit human checkpoint when substantive differences occur; insignificant PNG drift is intentionally not preserved in the release diff.
...
- Decision 2026-08-28 cli-owner-only-credential-file: Store CLI caller credentials in one owner-only file
    Decision: Store display-once caller API keys in `credentials.json` beside the selected CLI config, written atomically with mode `0600` inside an owner-only default config directory. Do not use an operating-system keyring, generate a separate encryption key, or retain migration and compatibility code for the unreleased keyring-backed format. `AGENT_OUTBOX_API_KEY` remains an explicit environment/secret-manager override.
    Reason: A self-encrypted file still requires a reliably stored master key, while desktop keyring behavior varies across Linux distributions, display servers, SSH sessions, containers, and macOS environments. A single protected file has no runtime service dependency and gives every supported installation the same observable behavior.
    Tradeoffs: The credential is protected by filesystem ownership and permissions rather than encryption at rest; users who require an external secret manager can inject the key through the environment. This pre-user cutover intentionally does not recover or migrate credentials written by development builds.

- Decision 2026-09-04 last-action-undo-no-toasts: Keep last-action undo in chrome and delete the toast layer
    Decision: Remove Sonner and all toast surfaces. A successful answer exposes one last-action Undo control in the application bar; History remains the place to reverse older eligible answers. Failures restore the affected row and show a dismissible application-bar alert.
    Reason: A stacked overlay sat on the review action rail, especially on phones, while History already records answered work. Immediate misclick recovery still has to stay on the queue because undo dies once the caller reads the result.
    Tradeoffs: There is no overlay history of the last several actions and no loading toast; rapid recovery is last-action only, and older undos require History.

- Decision 2026-09-22 concise-node-test-output: Keep the full Node spec log and a short stdout summary
    ...
    Reason: On Node 24.18.0, dot drops test diagnostics and test stdout/stderr, while spec keeps them and also prints every passing name. Root-process stderr is not a test event, so it never reaches the spec file. A nested `node --test` that inherits `NODE_TEST_CONTEXT` skips every file and exits 0. The `NO_COLOR` warning comes from Agent Layer plus the Grok command wrapper, not from this repo.
