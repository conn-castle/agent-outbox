...
- [ ] Complete the first public CLI publication as `v0.3.0`: the production release now builds and uploads verified tagged GoReleaser archives, checks anonymous download access, and opens the generated cask PR in `conn-castle/homebrew-tap`; its repository-scoped GitHub App secrets are mirrored from the canonical shared SSM parameters. Before running it, finish the repository-history review, make the repository public, and merge the guarded tap allowlist change. After the tap install smoke passes, replace the landing page and API quickstart's invite-only installer copy with the verified Homebrew command.
