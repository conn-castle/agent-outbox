...
- [ ] Decide and implement the public CLI installation posture: either publish verified tagged binaries/Homebrew distribution from the existing GoReleaser package path or explicitly launch with a tested build-from-source workflow, then make the landing page, API docs, and README agree. Until that publication lands, the homepage and API quickstart already share an invite-only access posture and a public `/contact` request path; they must not advertise an unpublished installer.
