...
- Added non-publishing GoReleaser archive verification through `make package-check`/`make release-check`; numbered releases render Homebrew metadata from exact checksums through the project-owned Go tool without requiring Python or Node at runtime.
...
- [x] Complete the first public CLI publication as `v0.2.6`: publish the four verified archives and checksums, merge the guarded `conn-castle/homebrew-tap` cask, and verify an unauthenticated Linux archive download, checksum, and version smoke.
- [x] Verify `brew install --cask conn-castle/tap/agent-outbox` in a clean official Homebrew environment, then align the landing page, contact flow, README, and API quickstart on public CLI installation with invite-only caller connection.
...
- [x] Changed `conn-castle/agent-outbox` from private to public.
- [ ] Verify an unauthenticated clone plus all website, documentation, license, issue, and support links from the public repository.
