...
- GitHub Actions is the only deployment and rollback mutation path. Never run
  `wrangler deploy`, `wrangler versions upload`, `wrangler versions deploy`, or
  `wrangler rollback` locally; follow the [release runbook](../release.md).
  Application releases upload an inactive Worker version and change traffic with
  exact version IDs. They do not apply routes or crons; route and cron drift
  fails the release until an operator applies them with the runbook's
