...
| Caller credentials in Supabase Postgres and the local CLI credentials file | Hosted key id, prefix, last characters, keyed secret digest, lifecycle timestamps; local plaintext caller API key                                                                                           | Hosted caller authentication and local CLI access. The hosted service does not retain the plaintext caller secret. The CLI stores it in an owner-only local file or reads it from `AGENT_OUTBOX_API_KEY` when supplied by the operator's secret manager.                                                                                        |
...
| Stripe webhook ledger in Supabase Postgres                                 | Stripe event id/type, receipt and completion timestamps, temporary rollout-compatible processing status, and optional account linkage                                                                       | Completed-only idempotency records. The claim and billing mutation commit atomically; rollback leaves no durable intermediate row and permits retry. The status column remains temporarily for prior-release rollback compatibility. No raw payload, payment method, or card data is stored.                                                    |
| Cloudflare                                                                 | Request handling, trusted source IP, route/status/timing logs, Worker execution, contact-form name/email/topic/message, Web Analytics page/performance metrics                                              | Hosting, DNS, edge security, contact-form delivery and rate limiting, structured logs, and privacy-oriented traffic/performance measurement. Contact content passes through the Email Service binding but is not written to application storage or logs. Web Analytics does not use browser storage and discards source IP from analytics data. |
...
| Browser client-event endpoint                                              | Closed event name/category, generated request/error ids, route and status                                                                                                                                   | Narrow allowlisted browser, GitHub sign-in launch, human-action, upload, and UI-consistency failure signals only. It is not a product-analytics event stream.                                                                                                                                                                                   |
| Zoho Mail                                                                  | Contact name, sender/recipient address, subject/topic, message, attachments, delivery metadata                                                                                                              | Receives contact-form and direct-email messages for support, billing, privacy, security, abuse, copyright, and legal contact. Users are instructed not to send secrets, card data, or unnecessary queue content.                                                                                                                                |
| AWS Systems Manager Parameter Store                                        | Runtime/provider credentials and resource configuration                                                                                                                                                     | Canonical project store for managed secrets and environment-owned provider configuration. It is not a store for customer queue content or product analytics.                                                                                                                                                                                    |
...
| Completed Stripe webhook ledger rows                    | Eligible for pruning after 90 days. A transaction failure rolls back the unique-key claim and billing mutation together, leaving the event retryable.                                 | `src/server/cleanup.ts`; Stripe webhook retention migration                         |
...
| Cloudflare logs, Sentry events, and Zoho contact email  | Provider-configured operational retention; no app-owned duplicate copy. Contact-form content is not included in application logs.                                                     | Provider configuration and service runbooks                                         |
