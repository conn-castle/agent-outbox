...
requires reconciliation. A multi-page `output read --all` reports `unknown` when
an earlier page was accepted and a later page was rejected or ambiguous, so
assume returned results may already have been marked read and fetch them again
by stable result id or by repeating the read. In particular, retry `input send`
with the same `caller_item_id` and content only after checking the caller-scoped
input list; never invent a new id to work around an ambiguous response.
