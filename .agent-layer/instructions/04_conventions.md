...
- **Repository boundary:** This repository's tasks, documentation, monitoring inventories, implementations, and PRs concern Agent Outbox only. Enforce that boundary in delegated prompts and validate returned repository paths and PR targets before continuing. A skill or copied instruction naming a different project is a scope error, not authorization to cross repositories. Do not import sibling-project artifacts or use sibling repositories as this project's source of truth.
